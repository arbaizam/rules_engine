...
### `SparkRulesEngineRuntime.evaluate_dataframe(df, ruleset, fail_on_error=True, include_error_traceback=False, audit_level="full")`
...
4. Evaluate rules through the work path selected by `audit_level`. Minimal skips
   all trace/provenance rendering; standard builds only the first-match trace;
   full also builds match summaries and assignment events.
...
The first three business-result columns and `error` are emitted at every audit
level. Standard adds the `first_matched_rule*` and backward-compatible
`winning_rule*` columns. Full, the compatibility default, also adds
`matched_rules`, `last_matched_rule`, and `assignment_results`. Identity columns
are driver-side literals at every level and do not enlarge the Python UDF result.
...
result_df = spark_runtime.evaluate_dataframe(
    ...
    ruleset,
    fail_on_error=True,
    audit_level="minimal",  # minimal | standard | full
)
```

Use `minimal` for high-volume production outputs that need typed assignments,
match IDs, errors, and immutable execution identity. Use `standard` when the
first winning trace is also needed. Use `full` for investigations or governed
outputs that require all match summaries and per-assignment provenance. Full is
the default so existing consumers retain their current schema.
...
- audit-level identity, semantic diffs, and coverage diagnostics,
