...
rules_engine/
  ...
  literal_types.py        # literal value_type validation
  ...
  runtime.py              # row evaluator used by Spark's Python compatibility path
  serializer.py           # dataclasses <-> persisted payload rows
  spark_native.py         # native Spark expression compiler
  spark_runtime.py        # execution selection and Python UDF compatibility path
  spark_types.py          # shared Spark result schemas
...
### `SparkRulesEngineRuntime.evaluate_dataframe(df, ruleset, fail_on_error=True, require_native=False)`
...
1. Compile supported field/literal rules into native Spark column expressions.
2. Use the Python UDF compatibility path only for Python custom functions or
   row-level error semantics that cannot be represented natively.
3. Append `rules_engine_*` result columns.
4. On the compatibility path, raise if any row has `rules_engine_error` and
   `fail_on_error=True`.

Set `require_native=True` for high-volume production jobs. Planning then fails
with the compatibility reason instead of silently selecting the Python UDF.
...
1. Compile compatible field/literal conditions, assignments, rule ordering,
   stop behavior, and winning-rule trace output into Spark column expressions.
2. Execute that plan in Spark without a Python row UDF.
3. Fall back to the Python UDF for registered Python custom functions,
   missing source fields, nonzero tolerance, unsafe mixed numeric coercions,
   or behavior that requires row-level Python error capture.
4. On the fallback path, fail fast if any row-level evaluator error is produced
   and `fail_on_error` is true.

Because the common path is expressed as Spark columns, downstream projections
can prune unused outputs such as the winning trace when a job selects only
assignment fields.

The default is `fail_on_error=True`. This prevents row-level evaluator
exceptions from silently becoming false non-matches. On the UDF path this
performs an error-detection Spark action before returning the DataFrame; a later
consumer action can evaluate the UDF again. Use `require_native=True` for
performance-sensitive jobs, or explicitly account for this validation cost.

String-to-string `eq` and `ne` comparisons are strict and do not coerce
numeric-looking strings. Numeric NaN values follow Spark semantics: NaN equals
NaN, sorts above finite numeric values, and uses the same equality in
`in`/`not_in` and ordering in `between`/`not_between`.

When a literal declares `value_type`, its runtime value must agree with that
hint. Validation rejects mismatches before Spark planning so execution never
depends on ANSI-sensitive implicit casts.

Winning-trace `value` fields are human-readable display strings. Their exact
timestamp and floating-point formatting can differ on the Python compatibility
path and must not be parsed as a stable machine-readable value contract.
...
Spark tests run by default when a Spark session or local Java runtime is
available. They skip only when local Spark cannot start. To disable them
explicitly:

```powershell
$env:RULES_ENGINE_RUN_SPARK_TESTS = "0"
...
- v1 assumes `spark.sql.parser.escapedStringLiterals=false`, the modern Spark
  default.
- Spark runtime compiles supported field/literal rules to native Spark
  expressions and uses a Python UDF compatibility path for Python custom
  functions, nonzero tolerance, and row-level error capture. Use
  `require_native=True` to reject fallback for performance-sensitive jobs.
...
- `like` and `not_like` support SQL `%` and `_` wildcards. Escape-character
  semantics for literal `%` or `_` are not part of v1; patterns containing a
  backslash use the Python compatibility path.
