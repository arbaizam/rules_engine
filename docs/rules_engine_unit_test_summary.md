...
| UT-138 | Spark runtime | Critical | `test_spark_runtime.py::test_spark_runtime_evaluates_row_rule` | Evaluates single- and multi-match rows through Spark, including existing-target `old_value`, last-write-wins assignment output, and override provenance. | Databricks Spark |
...
| UT-143 | Spark runtime | Medium | `test_spark_runtime.py::test_spark_runtime_evaluates_literal_only_rule_without_source_dependencies` | Evaluates a literal-only rule with no source dependencies or assignments and verifies the null empty-sentinel output. | Databricks Spark |
