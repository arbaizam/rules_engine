...
| ST-007 | Publication gate | A failing embedded expected case prevents every repository write. |
| ST-008 | Spark output contract | Compact and full-audit success, no-match, and error rows have identical business results and exact ordered columns. |
| ST-009 | Full-audit trace | Every condition reports its condition ID, containing group ID, group operator, active state, pass state, and source columns. |
| ST-010 | Rule control flow | A matching `stop_on_match: true` rule commits its assignments and skips later rules. |
| ST-011 | Prior assignments | Later rules read committed values through `assigned`, while assignments within one rule share a pre-rule snapshot. |
| ST-012 | Null substitution | Numeric and text `default_if_null` values are applied before comparison, while explicit unresolved-null errors remain row errors. |
| ST-013 | Assignment history | Full audit records the original value, each proposed value, overrides, and the final effective assignment. |
| ST-014 | Typed worker values | Decimal, date, and nested struct assignments retain their Spark types and values across the Python-worker boundary. |
