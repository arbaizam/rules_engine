...
The suite contains **213 explicit test functions** and collects **259 pytest cases** after parameter expansion. **21 cases** exercise the live Spark worker boundary and require `RULES_ENGINE_RUN_SPARK_TESTS=1` plus a compatible Spark and Java runtime.
...
| In-memory runtime | 58 | Ordered evaluation, assignment chaining, atomic same-rule assignments, null defaults, errors, condition identity, audit provenance, worker safety, key metadata, and immutable assignment keys. |
...
| Spark runtime | 21 | Keyed result separation, scalar and atomic-struct assignment application, explicit null clearing, shared persistence, native typed results, column ordering and prefixes, assignment chaining, semantic preflight, audit output, and lazy fail-fast execution through a real worker. |
